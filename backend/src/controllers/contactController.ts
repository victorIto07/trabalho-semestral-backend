...
export const createContact = async (req: Request, res: Response) => {
  ...
  try {
    const query = `insert into contacts(id, name, phone_number, email, image_url)
values('${newContact.id}', '${newContact.name}', '${newContact.phone_number}', '${newContact.email}', '${newContact.image_url}')`;
    await NewQuery(query);
...
export const updateContact = async (req: Request, res: Response) => {
  ...
  set name = '${req.body.name}', phone_number = '${req.body.phone_number}', email = '${req.body.email}', image_url = '${req.body.image_url}'
  where id = '${contactId}'`;
