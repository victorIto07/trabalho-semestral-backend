...
export const prepararHeader = (usuario) => {
  ...
  const name = usuario.name.split(' ')[0];

  saudacoes.appendChild(createElementFromString(`<span>Bem vindo(a), <strong>${name[0].toUpperCase() + name.slice(1).toLowerCase()}</strong>!</span>`))
